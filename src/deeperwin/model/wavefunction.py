...
def evaluate_sum_of_determinants_psi(mo_matrix_up, mo_matrix_dn):
    LOG_EPSILON = 1e-8
...
class Wavefunction(hk.Module):
    ...
    def _calculate_psi(self, n_up: int, n_dn: int, r, R, Z, fixed_params: Optional[Dict] = None):
        fixed_params = fixed_params or {}
        diff_dist, features = self.input_preprocessor(n_up, n_dn, r, R, Z, fixed_params.get('input'))
        ...
        embeddings = self._calculate_embedding(diff_dist, features, n_up)
        mo_up, mo_dn = self._calculate_orbitals(diff_dist, embeddings, fixed_params, Z.shape[-1], n_up, n_dn)
        psi = evaluate_sum_of_determinants_psi(mo_up, mo_dn)
        # Jastrow factor to the total wavefunction
        if self.config.jastrow:
            psi = psi*jnp.exp(self.jastrow(embeddings, n_up))

        # Electron-electron-cusps
        if self.config.use_el_el_cusp_correction:
            psi = psi*jnp.exp(self._el_el_cusp(diff_dist.dist_el_el))
        return psi
    ...
    def init_for_multitransform(self):
        return self.__call__, (self.__call__, self._calculate_psi, self.get_slater_matrices, self._calculate_embedding, self._calculate_orbitals, self._calculate_cache)
...
def build_log_psi_squared(
    ...
    psi = lambda params, n_up, n_dn, *batch: model.apply[1](params, None, n_up, n_dn, *batch)
    get_slater_mat = lambda params, n_up, n_dn, *batch: model.apply[2](params, None, n_up, n_dn, *batch)
    get_cache = lambda params, n_up, n_dn, *batch: model.apply[5](params, None, n_up, n_dn, *batch)

    LOGGER.debug(get_param_size_summary(params))
 
    return log_psi_sqr, psi, get_slater_mat, get_cache, params, fixed_params
